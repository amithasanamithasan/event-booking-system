            ...
            <div class="box-body">
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>{{$book->phone}}</td>
                  <td>{{$book->email}}</td>
                  <td>{{$book->booking_date}}- ( {{ $book->booking_time }} )</td>
                  <td>{{$book->min_guest}}-{{ $book->max_guest }}</td>
                  <td>{!! $book->note !!}</td>
                  <td><a href="/event-details/{{$book->event_id}}">Event details</a></td>
                  <td class="text-center">@if ($book->status==0)<a class=" btn-primary btn-sm" href="{{ url('/admin/approved-booking',$book->id) }}">Approved</a>||@endif<a class=" btn-danger btn-sm" href="{{ url('/admin/delete-booking',$book->id) }}">Delete</a></td>
                </tr>
                <?php $sl++; ?>
                @endforeach
                </tbody>
